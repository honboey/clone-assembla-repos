            ...
            for document in space:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        with open(
                            f"data/documents/{file_name}.{file_extension}", "wb"
                        ) as file:
                    ...
                    print(f"Failed to produce download_url for {document}\n\n")
                    pass
            else:
                continue


def download_single_document(str):
    """
    Given the Assembla download URL for a document, download that document.
    """
    headers = {"X-Api-Key": f"{api_key}", "X-Api-Secret": f"{api_secret}"}
    ...
    if response.status_code == 200:
        ...
        if content_type:
            ...
            # Save the file to your local directory
            with open(f"data/documents/{file_name}.{file_extension}", "wb") as file:
                file.write(response.content)
            print(f"File downloaded as {file_name}.{file_extension}")
        else:
